...
int main(int argc, char *argv[]) {
...
#elif FPGA_RECEIVE
    // =========================================================================
    // FPGA RECEIVER - interrupt triggers DMA, CPU does useful work
    // =========================================================================
    ...
    // Initialize PLIC
    plic_result_t plic_res = plic_Init();
    if (plic_res != kPlicOk) {
    ...
    plic_res = plic_irq_set_priority(SERIAL_LINK_FIFO_INTR_EVENT, 1);
    if (plic_res != kPlicOk) {
        PRINTF("Failed to set priority\n");
        return EXIT_FAILURE;
    }

    plic_res = plic_irq_set_enabled(SERIAL_LINK_FIFO_INTR_EVENT, kPlicToggleEnabled);
    if (plic_res != kPlicOk) {
    ...
    // CPU does useful work while waiting for interrupt
    PRINTF("CPU doing useful work while waiting for data...\n");
    volatile uint32_t cpu_work_counter = 0;
    while (sl_fifo_intr_flag == 0) {
        cpu_work_counter++;
    }
    PRINTF("Interrupt fired! Launching DMA...\n");
    sl_dma_read(dma_buffer, (uint32_t *)SL_READ, NUM_WORDS);
